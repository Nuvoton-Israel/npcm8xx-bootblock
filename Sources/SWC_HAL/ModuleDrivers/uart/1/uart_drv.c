...
DEFS_STATUS UART_Init (UART_DEV_T devNum, UART_REDIRECTION_T mode, UART_BAUDRATE_T baudRate)
{
    ...
    /* Parameters check                                                                                    */
    /*-----------------------------------------------------------------------------------------------------*/
    DEFS_STATUS_COND_CHECK((devNum < UART_NUM_OF_MODULES), DEFS_STATUS_PARAMETER_OUT_OF_RANGE);
    ...
    switch (mode)
    {
        case UART_MODE4_HSP1_SI1____HSP2_SI2____UART1_s_SI1___UART3_s_SI2__UART2_s_HSP1:
        case UART_MODE6_HSP1_SI1____HSP2_SI2____UART1_s_SI1___UART3_s_SI2__UART2_s_HSP2:
        case UART_MODE7_HSP1_SI1____HSP2_UART2__UART1_s_HSP1__UART3_SI2:

            sp1 = TRUE;
            sp2 = TRUE;
            break;

        case UART_MODE5_HSP1_SI1____HSP2_UART2__UART1_s_HSP1__UART3_s_SI1:

            sp1 = TRUE;
            break;

        case UART_MODE1_HSP1_SI2____HSP2_UART2__UART1_s_HSP1__UART3_s_SI2:
        case UART_MODE2_HSP1_UART1__HSP2_SI2____UART2_s_HSP2__UART3_s_SI2:
        case UART_MODE3_HSP1_UART1__HSP2_UART2__UART3_SI2:
    ...
    if (mode < UART_RESERVED)
    {
        SET_REG_FIELD(SPSWC, SPSWC_SPMOD, mode & 0x7); /* redirection mode number in enum == value at register */
    }

    if (mode != UART_SKIP_MUX)
    {
        CHIP_Mux_Uart(devNum, CoreSP, sp1, sp2);
    }
...
UINT8 UART_GetC( UART_DEV_T devNum )
{
    ...
    /* Parameters check                                                                                    */
    /*-----------------------------------------------------------------------------------------------------*/
    DEFS_STATUS_COND_CHECK((devNum < UART_NUM_OF_MODULES), 0);
...
DEFS_STATUS UART_SetBaudrate(UART_DEV_T devNum, UART_BAUDRATE_T baudrate)
{
    ...
    /*-----------------------------------------------------------------------------------------------------*/
    return UART_SetDivisor(devNum, divisor);
...
UINT32 UART_GetBaudrate(UART_DEV_T devNum)
{
    ...
    return uart_clock / (16 * (divisor + 2));
                                        ...
                                        UART_irq_callback_t txCallback, void* txParam)
{
    ...
    DEFS_STATUS_COND_CHECK((devNum < UART_NUM_OF_MODULES), DEFS_STATUS_INVALID_PARAMETER);

    iir = REG_READ(IIR(devNum)) & 0xF;
