...
/* File Contents:                                                             */
/*   mc_drv_sweeps.c                                                          */
/* This file contains manual sweeps for DDR. User can enable tests from here in case of */
/*        DRAM failures and board failures that require manaual analyze.      */
/* Project:                                                                   */
/*        SWC HAL                                                             */
/*---------------------------------------------------------------------------------------------------------*/
...
/* Side effects:                                                                                           */
/* Description:                                                                                            */
/*                  This routine performs sweep of DQn in\out. Results at g_Table_Y_BitStatus              */
/*---------------------------------------------------------------------------------------------------------*/
DEFS_STATUS Sweep_DQn_Trim_l (DDR_Setup *ddr_setup, int SweepType, BOOLEAN DoCenter, UINT16 SweepBitMask)
{
	...
	for (samplePoint = startScan; samplePoint <= stopScan; samplePoint++)
	{
		...
		g_fail_rate_0 = 0; // Uncomment to check failure rate
		g_fail_rate_1 = 0; // Uncomment to check failure rate
...
#define IP_DQS_end (63 - 8)
	...
	for (int ibit = 0; ibit < 16; ibit++) // display on lane0 eye, the critical issue we have. no issue was found in lane1
	{
		...
		// get IP_DQS delay
		int center_point_y = (int)stored_IP_DQS[(UINT32)ibit >> 3];

		// get IP_DQn delay
		REG_WRITE(PHY_LANE_SEL, ((UINT32)ibit >> 3 * DQS_DLY_WIDTH) + (((UINT32)ibit & 0x7) << 8));
		int center_point_x = TWOS_COMP_7BIT_REG_TO_VALUE(READ_REG_FIELD(IP_DQ_DQS_BITWISE_TRIM, IP_DQ_DQS_BITWISE_TRIM_ip_dq_dqs_bitwise_trim_reg)) + 62 - 42;
