...
#define _READ_FIELD(reg, reg_size, size, position)  \
    (((reg)>>(position)) & (_CASTING_FROM(reg_size))((_CASTING_FROM(reg_size))(1UL<<(size))-1))

#define _SET_REG_FIELD(reg, size, position, value)  \
    REG_WRITE(_GET_REG(reg),(((REG_READ(_GET_REG(reg))) & (~(((_CASTING_FROM(_GET_REG_SIZE(reg)))(1UL<<(size))-1)<<(position)))) \
    | ((((_CASTING_FROM(_GET_REG_SIZE(reg)))(1UL<<(size))-1) & (value))<<(position))))

#define  _SET_REG_FIELD_SIZE(reg,size,position,value)   \
    ((size == 1) ? ((value == 1) ? _SET_REG_BIT_FIELD(_GET_REG(reg), position) : _CLEAR_REG_BIT_FIELD(_GET_REG(reg), position)) : \
                   _SET_REG_FIELD(_GET_REG(reg),size,position,value))

#define _SET_VAR_FIELD(var,size,position,value)                                                                                                     \
   ((sizeof(var) == 8) ?   ((var) = ((var) & (~((((UINT64)(1UL<<(size))-1)<<(position))))) | ((((UINT64)(1UL<<(size))-1) & (value))<<(position))) : \
   ((sizeof(var) == 4) ?   ((var) = ((var) & (~((((UINT32)(1UL<<(size))-1)<<(position))))) | ((((UINT32)(1UL<<(size))-1) & (value))<<(position))) : \
   ((sizeof(var) == 2) ?   ((var) = ((var) & (~((((UINT16)(1UL<<(size))-1)<<(position))))) | ((((UINT16)(1UL<<(size))-1) & (value))<<(position))) : \
                           ((var) = ((var) & (~((((UINT8) (1UL<<(size))-1)<<(position))))) | ((((UINT8) (1UL<<(size))-1) & (value))<<(position))))))
