...
#ifdef SHM_CAPABILITY_EXTENDED_8_AREA_PROTECTION
    return MAKE16(REG_READ(WIN_WR_PROT(window_num)), REG_READ(WIN_RD_PROT(window_num)));
#endif

#ifdef SHM_CAPABILITY_WIN_PROTECTION
    return (UINT16)READ_REG_FIELD(WIN_PROT, WIN_PROT_RWIN(window_num));
#endif
}
...
/* Parameters:                                                                                             */
/*                  window_num     - Shared Memory Window to be written.                                   */
/*                  val (bit mask) - Clear pending interrupt on:                                           */
/*                  - 0x00000001 (SHM_INT_ON_HOST_RD_PROTECT_ADDR), host read from SHM protected address.  */
/*                  - 0x00000002 (SHM_INT_ON_HOST_WR_PROTECT_ADDR), host write to SHM protected address.   */
/*                  - 0x00000008 (SHM_INT_ON_HOST_WR_TO_INDIRECT_SEM), host write to the indirect semaphore*/
/*                  - 0x00000010 (SHM_INT_ON_HOST_WR_WIN1_SEM), host write to SHM win1 semaphore.          */
/*                  - 0x00000020 (SHM_INT_ON_HOST_WR_WIN2_SEM), host write to SHM win2 semaphore.          */
/*                  - 0x00000040 (SHM_INT_ON_ANY_HOST_SHM_WRITE), host write to any unprotected SHM address*/
/*                  - 0x00001000 (SHM_INT_ON_HOST_WR_WIN3_SEM), host write to SHM win3 semaphore.          */
/*                  - 0x00002000 (SHM_INT_ON_HOST_WR_WIN4_SEM), host write to SHM win4 semaphore.          */
/*                  - 0x00010000 (SHM_INT_ON_HOST_RD_WIN1_CORE_OFFSET), host read from win1 core offset.   */
/*                  - 0x00020000 (SHM_INT_ON_HOST_WR_WIN1_HOST_OFFSET), host write to win1 host offset.    */
/*                  - 0x00040000 (SHM_INT_ON_HOST_RD_WIN2_CORE_OFFSET), host read from win2 core offset.   */
/*                  - 0x00080000 (SHM_INT_ON_HOST_WR_WIN2_HOST_OFFSET), host write to win2 host offset.    */
/*                  - 0x01000000 (SHM_INT_ON_HOST_RD_WIN3_CORE_OFFSET), host read from win3 core offset.   */
/*                  - 0x02000000 (SHM_INT_ON_HOST_WR_WIN3_HOST_OFFSET), host write to win3 host offset.    */
/*                  - 0x04000000 (SHM_INT_ON_HOST_RD_WIN4_CORE_OFFSET), host read from win4 core offset.   */
/*                  - 0x08000000 (SHM_INT_ON_HOST_WR_WIN4_HOST_OFFSET), host write to win4 host offset.    */
/*                                                                                                         */
/* Returns:         none                                                                                   */
/* Side effects:                                                                                           */
/* Description:                                                                                            */
...
)
{
    ...
    case SHM_WINDOW_ONE:
        if (enable)
        {
            SET_REG_BIT(HOFS_CTL, bit);
        }
        else
        {
            CLEAR_REG_BIT(HOFS_CTL, bit);
        }
        break;
    ...
    case SHM_WINDOW_THREE:
        if (enable)
        {
            SET_REG_BIT(HOFSE_CTL, bit);
        }
        else
        {
            CLEAR_REG_BIT(HOFSE_CTL, bit);
        }
        break;
#endif
